...
String.prototype.realWordsInString = function(dictionary) {
    ...
    dictionary.forEach((el) => {
        if (this.includes(el)) foundWords.push(el);
    });
...
function anagrams(str1,str2) {
    ...
    // anonymous function would be fat arrow or declaring function, it's anonymous because it doesnt have a name and can't be declared elsewhere 
        // The difference here though, is that the arrow function won't have it's own THIS, it will have the outer context, important in monkey patching. You can't use arrow function to monkey patch. It would become window or the next outer thing.
    ...
    // find an explanation to the syntax for this last part. Want to know exactly what this is instead
    // of just assuming. I believe we can delcare a variable then create a function for that variable?
    // What is another way to write this last line too?
}
...
String.prototype.symmetricSubstrings = function() {
    ...
    arr.forEach((el) => {
        let reverseable = el.split('').reverse().join('');
        if ((el.length > 1) && (el === reverseable)) final.push(el)
    });
    return final.sort();
};
...
function titleize(str) {
    ...
    const nonCapWords = ["a", "and", "of", "over", "the"]

    const words = str.split(' ');

    // titleizedWords = words.map((word, index) => {
    //     if (index !==0 && nonCapWords.indexOf(word) > 0) {
...
Array.prototype.mergeSort = function(comparator) {
    ...
    if(!comparator) comparator = (left, right) => {
        return left < right ? -1 : left > right ? 1 :0
    }


    let mid = Math.floor(this.length / 2)
    let sortedLeft = this.slice(0,mid).mergeSort(comparator)
    let sortedRight = this.slice(mid).mergeSort(comparator)
...
Array.prototype.bubbleSort = function (callback) {
    ...
    while (!sorted) {
      ...
      for (let i = 0; i < resultArr.length - 1; i++) {
        ...
        let next = resultArr[i+1]
        if (callback(current, next) === 1) {
          sorted = false;
          resultArr[i] = next
          resultArr[i+1] = current
        }
      }
    }
    return resultArr;
...
function recSum(numArr) {
    ...
    // [1] = 1
    // [1,2] = 3
    // [1,2,3] = 6 = numArr.pop() + recSum(numArr)
...
function fibsSum(n) {
    ...
    //f(1) => [1] = 1     
    //f(2) => [1,1] = 2 = f(n-1) + 1     
    // f(3) => [1,1,2] = 4 = f(n-1) + f(n-2) + 1
    // f(4) => [1,1,2,3] = 7 = f(n-1) + f(n-2) + 1   
...
function stringIncludeKey(string, key) {
    ...
    // s(1) = slice(0,1) = c
    // s(2) = slice(0,2) + s(1) = c + a
    // s(3) = slice(0,3) + s(2) = ca + d
...
Array.prototype.median = function() {
    ...
    if (this.length % 2 === 1) {
        ...
        return (this.sort()[mid-1] + this[mid])/2
    }
}
...
String.prototype.mySlice = function(startIdx, endIdx) {
    ...
    let newStr = ''
    if(startIdx > endIdx) return newStr

    if(!endIdx) {
        endIdx = this.length
    }

    for(i = startIdx; i < endIdx && i < this.length; i++) {
        newStr += this[i]
    }
...
Array.prototype.myRotate = function(times = 1) {
    let splitIdx = times % this.length;
    const left = this.slice(0,splitIdx);
    const right = this.slice(splitIdx);
...
function myReverse(array) {
    ...
    while(i < array.length) {
        let spliced = array.splice(0)
        array.concat(spliced)
        i += 1
        debugger
    }
    return array;
...
Function.prototype.myBind = function(context, ...bindArgs) {
    ...
    return function(...callArgs) {
        return originalThis.apply(context, bindArgs.concat(callArgs))
    }
}
